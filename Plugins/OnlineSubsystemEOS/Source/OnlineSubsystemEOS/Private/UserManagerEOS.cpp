...
void FUserManagerEOS::LoginWithDeviceID()
{
	UE_LOG(LogTemp, Log, TEXT("Logging in with device ID"));
	...
	LoginInfo.DisplayName = "DisplayName";

	EOS_Connect_LoginOptions Options;
	Options.ApiVersion = EOS_CONNECT_LOGIN_API_LATEST;
	Options.UserLoginInfo = &LoginInfo;
	...
	CallbackObj->CallbackLambda = [this, LocalUserNum](const EOS_Connect_LoginCallbackInfo* Data)
	{
		...
		{
			UE_LOG(LogTemp, Log, TEXT("Successfully logged in with device ID"));
			
			FOnlineAccountCredentials OnlineAccountCredentials;
			OnlineAccountCredentials.Type = "deviceID";
		...
		{
			UE_LOG(LogTemp, Error, TEXT("LoginWithDeviceID() failed with error code %hs"), EOS_EResult_ToString(Data->ResultCode));
		}
	};
	
	EOS_Connect_Login(EOSSubsystem->ConnectHandle, &Options, (void*)CallbackObj, CallbackObj->GetCallbackPtr());
}
...
void FUserManagerEOS::CreateDeviceID()
{
	...
	CallbackObj->CallbackLambda = [this](const EOS_Connect_CreateDeviceIdCallbackInfo* Data)
	{
		...
		{
			UE_LOG(LogTemp, Log, TEXT("Device ID created successfully"));
			LoginWithDeviceID();
		}
		else
		{
			UE_LOG(LogTemp, Error, TEXT("CreateDeviceID() failed: %hs"), EOS_EResult_ToString(Data->ResultCode));
		}
	};

	EOS_Connect_CreateDeviceId(EOSSubsystem->ConnectHandle, &Options, (void*)CallbackObj, CallbackObj->GetCallbackPtr());
}
...
void FUserManagerEOS::DeleteDeviceID() const
{
	...
	EOS_Connect_OnDeleteDeviceIdCallback Callback = [](const EOS_Connect_DeleteDeviceIdCallbackInfo* Data)
	{
		if (Data->ResultCode == EOS_EResult::EOS_Success)
		{
			UE_LOG(LogTemp, Log, TEXT("Device ID deleted successfully"));
		}
		else
		{
			UE_LOG(LogTemp, Error, TEXT("DeleteDeviceID() failed %hs"), EOS_EResult_ToString(Data->ResultCode));
		}
	};

	EOS_Connect_DeleteDeviceId(EOSSubsystem->ConnectHandle, &Options, nullptr, Callback);
}
...
void FUserManagerEOS::CallEOSAuthLogin(int32 LocalUserNum, const FOnlineAccountCredentials& Credentials)
{
	...
	CallbackObj->CallbackLambda = [this, LocalUserNum, Credentials, bIsPersistentLogin](const EOS_Auth_LoginCallbackInfo* Data)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("%ls"), (IdentityInterface->GetLoginStatus(0) == ELoginStatus::LoggedIn ? TEXT("Logged In") : TEXT("Not Logged In")));
	};
